...
- `muesli-cli spec`
- `muesli-cli info`
- `muesli-cli transcribe <file> [--format text|json|markdown] [--model MODEL] [--summarize] [--save-meeting] [--title TITLE] [--output PATH] [--dictionary PATH] [--emit-partials PATH]`
...
Supported transcribe models are listed by `muesli-cli transcribe --help`; `gigaam-onnx` is the default. `--emit-partials` is available only with `parakeet-eou-320ms`.
